...
export async function POST(req: Request) {
  try {
    ...
    // This assumes the user in the session is an Admin in the database.
    // If your auth provider returns a user ID that matches the Admin ID, use that.
    // Otherwise, look up the Admin by email.
...
export async function PUT(req: Request) {
  try {
    ...
    const body = await req.json();
    const { id, title, slug, content, published, categoryIds, tagIds } = body;
